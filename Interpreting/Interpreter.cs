...
namespace Lab4.Interpreting {
	sealed class Interpreter : IStatementVisitor, IExpressionVisitor<object> {
		...
		public void VisitArrayAssignment(ArrayAssignment arrayAssignment) {
			var array = Calc<List<object>>(arrayAssignment.Array);
			int left = Calc<int>(arrayAssignment.Left);
			int right = Calc<int>(arrayAssignment.Right);
			var expr = Calc<List<object>>(arrayAssignment.Expr);
			if (!(right <= array.Count && left <= right)) {
				throw MakeError(arrayAssignment.Right, $"Правый индекс должен быть больше или равно левого и меньше размерности массива {arrayAssignment.Right}");
			}
			array.RemoveRange(left, right - left);
			array.InsertRange(left, expr);
		}
		#endregion
		#region expressions
		...
		public object VisitArrayExpr(ArrayExpr arrayExpr) {
			return arrayExpr.Items.Select(Calc).ToList();
		}
		#endregion
